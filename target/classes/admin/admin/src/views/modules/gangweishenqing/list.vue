<template>
	<div class="main-content">
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px","margin":"0 20px 0 0","background":"none","display":"flex","width":"100px","justifyContent":"space-between","order":"1"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"padding":"0","margin":"110px 0 0","flexWrap":"wrap","background":"none","display":"block","width":"100px","justifyContent":"center"}'>
					<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 0px 16px 0","borderColor":"#812d58","color":"#333","minWidth":"100px","outline":"none","borderRadius":"0px","background":"linear-gradient(180deg, rgba(255,255,255,.1) 0%, rgba(201,238,222,.1) 58%, rgba(113,190,157,.3) 60%, rgba(93,174,139,.3) 100%)","borderWidth":"0px","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}' v-if="isAuth('gangweishenqing','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 0px 16px 0","borderColor":"#f07062","color":"#333","minWidth":"100px","outline":"none","borderRadius":"0px","background":"linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(255,221,221,.1) 58%, rgba(255,150,150,.3) 60%, rgba(255,118,118,.3) 100%)","borderWidth":"0px","width":"auto","fontSize":"14px","borderStyle":"dashed","height":"40px"}' v-if="isAuth('gangweishenqing','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>




					<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 0px 16px 0","borderColor":"#c9a160","color":"#333","minWidth":"100px","outline":"none","borderRadius":"0px","background":"linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(222,255,255,.1) 58%, rgba(136,242,242,.3) 60%, rgba(63,208,208,.3) 100%)","borderWidth":"0px","width":"auto","fontSize":"14px","borderStyle":"dashed","height":"40px"}' v-if="isAuth('gangweishenqing','岗位申请统计')" type="warning" @click="chartDialog1()">岗位申请统计</el-button>
				</el-row>
			</el-form>
			...
			<!-- <div> -->
				<el-table class="tables"
					...
					v-if="isAuth('gangweishenqing','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('gangweishenqing','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 10px 10px","borderColor":"#5dae8b","color":"#268e61","outline":"none","borderRadius":"0px","background":"url()","borderWidth":"0 0 4px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if=" isAuth('gangweishenqing','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 10px 10px","borderColor":"#5dae8b","color":"#268e61","outline":"none","borderRadius":"0px","background":"url()","borderWidth":"0 0 4px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if="isAuth('gangweishenqing','面试邀请')" type="success" size="mini" @click="gangweimianshiCrossAddOrUpdateHandler(scope.row,'cross','是','','')">面试邀请</el-button>
							<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 10px 10px","borderColor":"#1b89ef","color":"#1b89ef","outline":"none","borderRadius":"0px","background":"none","borderWidth":"0 0 4px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if=" isAuth('gangweishenqing','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 10px 10px","borderColor":"#eb4a5f","color":"#eb4a5f","outline":"none","borderRadius":"0px","background":"none","borderWidth":"0 0 4px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if="isAuth('gangweishenqing','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<gangweimianshi-cross-add-or-update v-if="gangweimianshiCrossAddOrUpdateFlag" :parent="this" ref="gangweimianshiCrossaddOrUpdate"></gangweimianshi-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="gangweimingchengChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import gangweimianshiCrossAddOrUpdate from "../gangweimianshi/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      gangweimianshiCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    gangweimianshiCrossAddOrUpdate,
  },
  methods: {
    ...
    gangweimianshiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.gangweimianshiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','gangweishenqing');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.gangweimianshiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.gangweimianshiCrossaddOrUpdate.init(row.id,type);
      });
    },
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        var gangweimingchengChart1 = echarts.init(document.getElementById("gangweimingchengChart1"),'macarons');
        this.$http({
            url: "gangweishenqing/group/gangweimingcheng",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                gangweimingchengChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    gangweimingchengChart1.resize();
                };
            }
        });
      })
