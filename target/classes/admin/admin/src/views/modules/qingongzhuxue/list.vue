<template>
	<div class="main-content">
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px","margin":"0 20px 0 0","background":"none","display":"flex","width":"100px","justifyContent":"space-between","order":"1"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0","borderColor":"#fde2db #fbccc0 #f58c71","margin":"0","textAlign":"center","display":"flex","justifyContent":"center","top":"100px","flexWrap":"wrap","left":"20px","background":"none","borderWidth":"0px","width":"calc(100% - 20px)","position":"absolute","borderStyle":"solid"}' >
					<div :style='{"width":"auto","margin":"0 10px 0 0","background":"none","display":"inline-block"}'>
						<label :style='{"padding":"0 4px 0 0","color":"#666","textAlign":"right","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">岗位名称</label>
						<el-input v-model="searchForm.gangweimingcheng" placeholder="岗位名称" clearable></el-input>
					</div>
				...
				<el-row :style='{"padding":"0","margin":"110px 0 0","flexWrap":"wrap","background":"none","display":"block","width":"100px","justifyContent":"center"}'>
					<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 0px 16px 0","borderColor":"#812d58","color":"#333","minWidth":"100px","outline":"none","borderRadius":"0px","background":"linear-gradient(180deg, rgba(255,255,255,.1) 0%, rgba(201,238,222,.1) 58%, rgba(113,190,157,.3) 60%, rgba(93,174,139,.3) 100%)","borderWidth":"0px","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}' v-if="isAuth('qingongzhuxue','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 0px 16px 0","borderColor":"#f07062","color":"#333","minWidth":"100px","outline":"none","borderRadius":"0px","background":"linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(255,221,221,.1) 58%, rgba(255,150,150,.3) 60%, rgba(255,118,118,.3) 100%)","borderWidth":"0px","width":"auto","fontSize":"14px","borderStyle":"dashed","height":"40px"}' v-if="isAuth('qingongzhuxue','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>




					<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 0px 16px 0","borderColor":"#c9a160","color":"#333","minWidth":"100px","outline":"none","borderRadius":"0px","background":"linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(222,255,255,.1) 58%, rgba(136,242,242,.3) 60%, rgba(63,208,208,.3) 100%)","borderWidth":"0px","width":"auto","fontSize":"14px","borderStyle":"dashed","height":"40px"}' v-if="isAuth('qingongzhuxue','岗位类型统计')" type="warning" @click="chartDialog1()">岗位类型统计</el-button>
				</el-row>
			</el-form>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="bangonghuanjing" width="200" label="办公环境">
						<template slot-scope="scope">
							<div v-if="scope.row.bangonghuanjing">
								<img v-if="scope.row.bangonghuanjing.substring(0,4)=='http'" :src="scope.row.bangonghuanjing.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.bangonghuanjing.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('qingongzhuxue','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 10px 10px","borderColor":"#5dae8b","color":"#268e61","outline":"none","borderRadius":"0px","background":"url()","borderWidth":"0 0 4px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if=" isAuth('qingongzhuxue','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 10px 10px","borderColor":"#5dae8b","color":"#268e61","outline":"none","borderRadius":"0px","background":"url()","borderWidth":"0 0 4px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if="isAuth('qingongzhuxue','岗位申请')" type="success" size="mini" @click="gangweishenqingCrossAddOrUpdateHandler(scope.row,'cross','是','[1]','已申请')">岗位申请</el-button>
							<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 10px 10px","borderColor":"#1b89ef","color":"#1b89ef","outline":"none","borderRadius":"0px","background":"none","borderWidth":"0 0 4px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if=" isAuth('qingongzhuxue','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"cursor":"pointer","padding":"0 0px","margin":"0 10px 10px","borderColor":"#eb4a5f","color":"#eb4a5f","outline":"none","borderRadius":"0px","background":"none","borderWidth":"0 0 4px","width":"auto","fontSize":"14px","borderStyle":"double","textShadow":"0 0px 0 rgba(0,0,0,.25)","height":"32px"}' v-if="isAuth('qingongzhuxue','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<gangweishenqing-cross-add-or-update v-if="gangweishenqingCrossAddOrUpdateFlag" :parent="this" ref="gangweishenqingCrossaddOrUpdate"></gangweishenqing-cross-add-or-update>
...
import gangweishenqingCrossAddOrUpdate from "../gangweishenqing/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      gangweishenqingCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    gangweishenqingCrossAddOrUpdate,
  },
  methods: {
    ...
    gangweishenqingCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.gangweishenqingCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','qingongzhuxue');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.gangweishenqingCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.gangweishenqingCrossaddOrUpdate.init(row.id,type);
      });
    },
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "qingongzhuxue/group/gangweileixing",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          gangweileixing: row.gangweileixing,
          xinzidaiyu: row.xinzidaiyu,
          gongzuoshijian: row.gongzuoshijian,
          gongzuodidian: row.gongzuodidian,
          gangweijieshao: row.gangweijieshao,
          lianxiren: row.lianxiren,
          lianxidianhua: row.lianxidianhua,
          bumenzhanghao: row.bumenzhanghao,
          bumenmingcheng: row.bumenmingcheng,
          sfsh: row.sfsh,
          shhf: row.shhf,
          id: row.id
        }
